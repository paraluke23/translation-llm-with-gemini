...
from typing import Any, Literal, Optional, Union, List, Dict # Added Optional, Union, List, Dict
...
class Server:
    ...
    async def initialize(self) -> None:
        ...
        if not command:
            raise EnvironmentError("Python interpreter not found in PATH.")
        ...
        try:
            logging.debug(f"Starting server '{self.name}' with command: {command} {server_script_path}")
    ...
    async def list_tools(self) -> list['Tool']: # <<< MODIFIED TYPE HINT HERE >>>
        ...
        tools = []
        logging.debug(f"Raw tools response from server: {tools_response}") # Added debug log
        ...
        for item in tools_response:
            ...
            if isinstance(item, tuple) and item[0] == "tools":
                ...
                if not isinstance(tool_spec_list, list):
                    logging.warning(f"Expected a list of tools, but got: {type(tool_spec_list)}")
                    continue
                ...
                for tool_spec in tool_spec_list:
                    ...
                    tool_schema = getattr(tool_spec, 'input_schema', getattr(tool_spec, 'inputSchema', {})) # Try both common names

                    if tool_name: # Only add if we could get a name
                        tools.append(Tool(tool_name, tool_desc, tool_schema))
                    else:
                        logging.warning(f"Could not extract name from tool spec object: {tool_spec}")
    ...
    async def execute_tool(
        ...
        while attempt < retries + 1:
            try:
                logging.debug(f"Attempt {attempt + 1}: Executing {tool_name} on server {self.name}...")
                ...
                if attempt < retries + 1:
                    ...
                    raise RuntimeError(f"Tool execution failed after {retries+1} attempts for tool '{tool_name}'") from last_exception
...
class Tool:
    ...
    def __init__(
        self, name: str | None, description: str | None, input_schema: dict[str, Any] | None
    ) -> None:
...
class LLMClient:
    ...
    def _initialize_client(self):
        """Initializes the GenAI client if not already done."""
        if not self._client:
            try:
                ...
                logging.exception("Failed to initialize GenAI client.")
                raise ConnectionError("Could not initialize connection to the LLM service.") from e
    ...
    def set_system_instruction(self, system_instruction: str) -> None:
        ...
        if not self._client: # Should not happen if _initialize_client works, but belt-and-suspenders
            raise ConnectionError("LLM Client not initialized.")

        try:
            # Create a new chat session with the system instruction
            # Note: Depending on the exact genai version, system instructions might be
            ...
            logging.info("LLM chat session initialized.") # System instruction set is implied

        except Exception as e:
            logging.exception("Failed to create LLM chat session or set system instruction.")
    ...
    def extract_tool_call_json(text: str) -> Optional[Dict[str, Any]]:
        ...
        if match:
            ...
            if text_stripped.startswith('{') and text_stripped.endswith('}'):
                ...
                logging.debug("No ```json block found, attempting to parse entire stripped text as JSON.")
        ...
        if not json_string:
            ...
            if text.strip(): # Only log if there was actual text content
                logging.debug(f"Could not extract a JSON string from the LLM response: >>>{text}<<<")
            return None
        ...
        try:
            ...
            if isinstance(loaded_json, dict) and "tool" in loaded_json and "arguments" in loaded_json:
                logging.debug("Successfully parsed JSON and validated tool call structure.")
                return loaded_json
            else:
                logging.debug("Parsed JSON but it does not match expected tool call structure.")
                return None # Not a valid tool call structure
        except json.JSONDecodeError as e:
            logging.warning(f"Error decoding JSON: {e}. String was: >>>{json_string}<<<")
            return None
...
class ChatSession:
    ...
    def __init__(self, gemini_server: Server, llm_client: LLMClient) -> None:
        ...
        self.messages: List[Dict[str, Any]] = [] # Use Any for content type flexibility (str or dict)
    ...
    async def cleanup_servers(self) -> None:
        ...
        if self.gemini_server:
            ...
            try:
                ...
                logging.warning(f"Warning during server cleanup ({self.gemini_server.name}): {e}")
    ...
    async def _prepare_llm(self) -> bool:
        ...
        try:
            ...
            if not self.available_tools:
                logging.warning(f"No tools found on server {self.gemini_server.name}. Interaction will be limited.")
                # Decide if you want to proceed without tools or exit
                # return False # Example: Exit if no tools
            else:
                logging.info(f"Available tools: {[tool.name for tool in self.available_tools]}")
            ...
            self.messages = [{"role": "system", "content": system_instruction_content}] # Adjust role if needed ('system'/'user'/'context')

            logging.info("LLM client and system prompt prepared successfully.")
            return True

        except (FileNotFoundError, json.JSONDecodeError, ValueError, EnvironmentError, ConnectionError) as e:
    ...
    async def _execute_tool_and_get_result(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        if not tool_exists:
            error_msg = f"Error: Tool '{tool_name}' is not listed as available on the server."
        ...
        try:
            ...
            logging.info(f"Tool '{tool_name}' execution successful. Result: {result_str}")
        ...
        while True:
            try:
                ...
                if parsed_tool_call:
                    ...
                    # 5. Execute the tool
                    tool_result_content = await self._execute_tool_and_get_result(tool_name, arguments)
                    ...
                    self.messages.append({"role": "user", "content": f"Tool execution result for {tool_name}: {tool_result_content}"}) # Providing context
                ...
                print(f"Assistant: Sorry, I'm having trouble connecting to the service. Please try again later.")
                break # Exit loop on connection errors
            except Exception as e:
                logging.exception("An unexpected error occurred in the main chat loop:")
                print(f"Assistant: Sorry, an unexpected error occurred ({e}). Please try again.")
...
async def main() -> None:
    ...
    if not gemini_server_config_data or "config" not in gemini_server_config_data:
        logging.error("Configuration for 'geminiServer' is missing or incomplete in servers_config.json")
        return
